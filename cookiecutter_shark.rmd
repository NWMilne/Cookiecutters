...
Gmm2 <-
  gamm4(
    fresh ~ temp + s(day) + s(lunar_illum, k = 8),
    random = ~ (1 | cluster),
    offset(body),
    family = binomial(link = "cloglog"),
    data = bites
...
AIC(Gmm1$mer, Gmm1a$mer, Gmm1b$mer, Gmm1c$mer, Gmm1d$mer, Gmm1e$mer, Gmm1f$mer, Gmm2$mer)
BIC(Gmm1$mer, Gmm1a$mer, Gmm1b$mer, Gmm1c$mer, Gmm1d$mer, Gmm1e$mer, Gmm1f$mer, Gmm2$mer)

summary(Gmm1f$gam)
anova(Gmm1f$gam)
plot.gam(Gmm1f$gam, page=1)
res <- residuals(Gmm1f$gam, type="deviance")
plot(res)
...
#AUC
```{r AUC}
gamm_df <- data.frame(fresh, temp, cluster, day, body, lunar_illum)

# Generate AUC
prob <- predict(Gmm1f$gam, newdata = gamm_df, type = "response")
prob <- as.data.frame(prob)
pred <- prediction(prob, gamm_df$fresh)
...
cutoff <- optimalCutoff(gamm_df$fresh, prob$prob, "Both")

# Generate confusion matrix
caret::confusionMatrix(data = factor(prob$prob > cutoff), factor(gamm_df$fresh == 1),
                positive = "TRUE")
```
...
names(bites)
#Gmm2 <-   gamm4(fresh ~ temp + s(day) + s(lunar_illum, k = 8),random = ~ (1 | cluster), offset(body), amily = binomial(link = "cloglog"), data = bites )  
